...
filter_history() {
    ...
    echo "Removing the following paths from git history:"
    for path in "$@"; do
        echo "  - $path"
    done
    ...
    git filter-repo $filter_paths --invert-paths --force &>/dev/null
...
setup_remote() {
    ...
    git remote add origin git@github.com:mason-jing/mason-jing.github.io.git &>/dev/null || \
    git remote set-url origin git@github.com:mason-jing/mason-jing.github.io.git
    echo "✅ Remote repository added"

    echo "Fetching remote branches..."
    git fetch origin &>/dev/null || echo "⚠️  Could not fetch from remote (may not exist yet)"
...
commit_and_cleanup() {
    ...
    if git diff --cached --quiet; then
        ...
        git commit -m "feat: restore files with safe content" &>/dev/null || true
...
verify_changes() {
    ...
    for path in "$@"; do
        # Check if path exists in history, excluding the restore commit
        if git log --all --full-history --grep="feat: restore files with safe content" --invert-grep -- "$path" &>/dev/null | grep -q .; then
...
force_push_branches() {
    ...
    local branches=$(git branch | sed 's/^[[:space:]]*\*[[:space:]]*//g' | sed 's/^[[:space:]]*//g' | grep -v "^backup-before-clean$")
    echo "$branches"
    echo ""
    echo "Force pushing all branches:"
    for branch in $branches; do
        echo "  Pushing $branch..."
        git push origin "$branch" --force-with-lease &>/dev/null && echo "  ✅ Pushed: $branch" || echo "  ⚠️  Failed to push: $branch"
    done
...
force_push_branches
echo ""
echo "Deleting backup branch: backup-before-clean"
git branch -D backup-before-clean &>/dev/null || true
